...
function getUpdateNotificationTitle(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return `OpenTake ${payload.version} is available`;
		case "downloading":
			return `Downloading OpenTake ${payload.version}`;
		case "ready":
			return `OpenTake ${payload.version} is ready`;
		case "error":
			return `OpenTake ${payload.version} needs attention`;
	}
}
...
function getUpdateNotificationBody(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return "Click to install the update and restart OpenTake.";
		case "downloading":
			return "OpenTake is downloading the update and will restart when it is ready.";
		case "ready":
...
function updateTrayMenu(recording: boolean = false) {
	...
	const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "OpenTake";
...
app.whenReady().then(async () => {
	if (process.platform === "win32") {
		app.setAppUserModelId("dev.opentake.app");
	}
	...
	// OpenTake does not use WebHID, Web Serial, or WebUSB. Do not grant devices by default.
