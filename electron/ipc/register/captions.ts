	...
	ipcMain.handle("open-video-file-picker", async (_, options?: OpenVideoFilePickerOptions) => {
		try {
			...
			const result = await dialog.showOpenDialog({
				title: includeProjects ? "Import Media or OpenTake Project" : "Select Video File",
				defaultPath: recordingsDir,
				filters: [
					...(includeProjects
						? [
								{
									name: "Media or OpenTake Projects",
									extensions: [
					...
					...(includeProjects
						? [{ name: "OpenTake Projects", extensions: PROJECT_FILE_EXTENSIONS }]
						: []),
