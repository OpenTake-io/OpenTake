...
async function resolveExperimentalWindowsGpuExporterPath() {
	...
	const executableNames = ["opentake-gpu-export.exe", "gpu-export-probe.exe"];
...
export async function exportNativeStaticLayoutVideo(
	...
	const sessionId =
		options.sessionId ??
		`opentake-static-layout-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
...
export async function probeNativeVideoEncoder(
	...
	const outputPath = path.join(
		app.getPath("temp"),
		`opentake-export-probe-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mp4`,
	);
...
export async function muxNativeVideoExportAudio(
	...
	if (audioMode === "edited-track" && !useEditedTrackFiltergraph) {
		...
		audioInputPath = path.join(
			app.getPath("temp"),
			`opentake-export-audio-${Date.now()}-${Math.random().toString(36).slice(2, 8)}${extension}`,
		);
...
export async function muxExportedVideoAudioBuffer(
	...
	const tempVideoPath = path.join(
		app.getPath("temp"),
		`opentake-export-video-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mp4`,
	);
	const metrics: NativeVideoAudioMuxMetrics = {};
	let succeeded = false;
