...
describe("isInternalRendererTarget", () => {
	it.each([
		...
		[
			"file:///opt/OpenTake/dist/index.html?windowType=editor",
			"file:///opt/OpenTake/dist/index.html?windowType=hud-overlay#status",
		],
	...
	it.each([
		...
		["https://opentake.example/", "http://opentake.example/"],
		["https://opentake.example/", "https://user:pass@opentake.example/"],
		["file:///opt/OpenTake/dist/index.html", "file:///etc/passwd"],
		["file:///opt/OpenTake/dist/index.html", "data:text/html,hello"],
...
describe("navigation event handlers", () => {
	...
	it("stops same-file query mutation without externalizing it", () => {
		...
		handler({
			url: "file:///opt/OpenTake/dist/index.html?smokeExport=1",
			preventDefault,
		});
	...
	it("stops unsafe schemes without opening them externally", () => {
		...
		const openExternal = vi.fn(async () => undefined);
		const handler = createWillNavigateHandler(
			() => "file:///opt/OpenTake/dist/index.html",
			openExternal,
		);
	...
	it("does not trust a renderer-mutated URL as an exact reload", () => {
		let currentUrl = "file:///opt/OpenTake/dist/index.html?windowType=editor";
		const on = vi.fn();
		const webContents = {
			getURL: () => currentUrl,
			on,
		...
		currentUrl = "file:///opt/OpenTake/dist/index.html?windowType=source-selector";
	...
	it("trusts an exact reload after a completed document navigation", () => {
		...
		const loadedUrl = "file:///opt/OpenTake/dist/index.html?windowType=editor";
