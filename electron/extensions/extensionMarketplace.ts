 ...
 * OpenTake marketplace API. Also provides admin review endpoints.
 */
...
const MARKETPLACE_API_BASE = "https://marketplace.opentake.dev/extensions/api/v1";
...
async function marketplaceFetch<T>(
	...
	try {
		const headers: Record<string, string> = {
			...
			"X-OpenTake-Version": app.getVersion(),
			"X-OpenTake-Platform": process.platform,
		};
...
export async function downloadAndInstallExtension(
	...
	const allowedOrigins = [
		"https://marketplace.opentake.dev",
		"https://opentake.dev",
	...
	const tempDir = path.join(app.getPath("temp"), `opentake-ext-${extensionId}-${Date.now()}`);
	...
	try {
		...
		if (dirs.length === 1 && !existsSync(path.join(extractDir, "opentake-extension.json"))) {
			manifestDir = path.join(extractDir, dirs[0].name);
		}

		// Verify manifest exists
		if (!existsSync(path.join(manifestDir, "opentake-extension.json"))) {
			throw new Error(
				"Downloaded extension does not contain a opentake-extension.json manifest",
			);
		}
		...
		fetch(`${getMarketplaceUrl()}/extensions/${encodeURIComponent(extensionId)}/download`, {
			method: "POST",
			headers: { "X-OpenTake-Version": app.getVersion() },
		}).catch(() => undefined);
